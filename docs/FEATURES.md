...
| 字幕 | フォント・サイズ・太さ・縁取り・透明度・影・グラデーション・背景、カラオケと区間タイミング、文字アニメーション、SRT、MP4焼き込み |
| モーション | 既存6テンプレート＋ベクター、テキスト・長方形・楕円・線・多角形・星、反復、線の描画／消去、モーションからキー展開、グラデーション、線幅、角丸、シャドウ、グロー、字間・太さ |
...
| Google Vids | 声・人物・演技・発声タグ付き台本を保存して引き渡し。Codexの認証済みブラウザで日本語音声生成を実証。ダウンロードしたMP4から圧縮音声を無変換抽出・取り込み |
...
声・字幕・モーションの操作と制約は [VOICE-MOTION.md](VOICE-MOTION.md)、AWS無料枠の判断は [HOSTING.md](HOSTING.md) を参照してください。
