...
export async function renderVectorPass(state,folder){
  if(!state.graphics?.some(g=>g.type==='vector'))return;
  ...
  const font=process.platform==='win32'?{loadSystemFonts:false,fontFiles:[path.join(process.env.WINDIR||'C:/Windows','Fonts/meiryo.ttc'),path.join(process.env.WINDIR||'C:/Windows','Fonts/meiryob.ttc')],defaultFontFamily:'Meiryo'}:{loadSystemFonts:true,defaultFontFamily:'sans-serif'};
  const proc=spawn(process.env.FFMPEG_PATH||'ffmpeg',['-hide_banner','-loglevel','error','-y','-threads','1','-i',source,'-f','image2pipe','-framerate',String(state.fps),'-i','pipe:0','-filter_complex_threads','1','-filter_complex','[0:v][1:v]overlay=shortest=1:format=auto,format=yuv420p[v]','-map','[v]','-map','0:a?','-c:v','libx264','-preset','veryfast','-crf','18','-threads','1','-c:a','copy','-movflags','+faststart',target],{windowsHide:true,stdio:['pipe','ignore','pipe']});
  ...
  try{
    ...
    for(let frame=0;frame<count;frame++){
      ...
      const svg=vectorSvg(state,frame/state.fps);
