...
export default memo(function Timeline({ state, clock, selected, onSelect, onUpdate, onSplit, onDelete, onArrange, onClear, onOverlaySelect, onOverlayUpdate, onOverlayDelete, busy }) {
  ...
  const drag = (event, item, kind) => {
    ...
    let offset = 0;
    const move = current => { offset = (current.clientX - startX) / scale; target.style.transform = `translateX(${Math.max(-item.start, offset) * scale}px)`; };
    const finish = current => {
      window.removeEventListener('pointermove', move); window.removeEventListener('pointerup', finish); window.removeEventListener('pointercancel', cancel); target.style.transform = '';
      if (Math.abs(offset * scale) <= 4) return;
      let start = Math.max(0, Math.round((item.start + offset) * state.fps) / state.fps);
      if(!current?.shiftKey){const points=[0,clock.time,...(state.markers||[]).map(m=>m.time),...state.clips.filter(c=>c.id!==item.id).flatMap(c=>[c.start,c.start+c.duration])];const targets=points.flatMap(t=>[t,t-item.duration]).filter(t=>t>=0&&Math.abs(t-start)*scale<8).sort((a,b)=>Math.abs(a-start)-Math.abs(b-start));if(targets.length)start=targets[0];}
  ...
  const remove = () => selectedClip ? onDelete(selectedClip) : overlaySelected ? onOverlayDelete(selected.type, selected.id) : null;
  const totalItems = state.clips.length + (state.captions?.length || 0) + (state.graphics?.length || 0);
  return <section className="timeline-panel"><div className="timeline-toolbar"><div className="timeline-tools"><span className="sequence-label">タイムライン</span><span className="toolbar-divider" /><Button icon={Scissors} action="timeline.split" title="再生ヘッドで分割 (S)" aria-label="再生ヘッドで分割" disabled={!selectedClip || !!busy} onClick={() => onSplit(selectedClip)} /><Button icon={Trash2} action="timeline.removeSelected" title="選択レイヤーを削除" aria-label="選択レイヤーを削除" disabled={(!selectedClip && !overlaySelected) || !!busy} onClick={remove} /><span className="toolbar-divider" /><Button icon={Layers3} action="timeline.arrange.open" disabled={!state.assets.some(a => a.decision === 'accepted' && a.sceneId) || !!busy} onClick={onArrange}>採用素材を整列</Button></div><div className="timeline-tools"><Button icon={RefreshCw} action="waveform.retry" title="波形を再取得" aria-label="波形を再取得" disabled={!state.clips.some(clip => clip.track === 'audio')} onClick={retryWaveforms} /><span className="timeline-duration">{timecode(clock.duration, state.fps)}</span><label className="zoom-control"><span>−</span><input type="range" aria-label="タイムラインのズーム" data-action="timeline.zoom" min="12" max="90" step="2" value={scale} onChange={event => setScale(Number(event.target.value))} /><span>+</span></label><Button icon={Trash2} title="映像と音声の配置を空にする" aria-label="映像と音声の配置を空にする" action="timeline.clear.open" disabled={!state.clips.length || !!busy} onClick={onClear} /></div></div><div className="timeline-scroller" ref={scrollRef}><div className="timeline-content" style={{ minWidth: width + 96, minHeight: 28 + rows.reduce((sum, row) => sum + (['graphic', 'caption'].includes(row.kind) ? 44 : 74), 0) }}><div className="timeline-labels"><div className="ruler-label">秒</div>{rows.map(row => <div key={row.id} className={['graphic', 'caption'].includes(row.kind) ? 'overlay-label' : ''}><span className={`track-letter ${row.kind}-letter`}>{row.label}</span><span>{row.title}</span><row.Icon size={11} /></div>)}</div><div className="timeline-lanes" ref={laneRef} style={{ width }} onPointerDown={seek}><div className="timeline-ruler">{ticks.map(tick => <span key={tick} style={{ left: tick * scale }}>{timecode(tick).slice(3, 8)}</span>)}</div>{rows.map(row => <div key={row.id} className={`timeline-lane ${row.kind}-lane ${['graphic', 'caption'].includes(row.kind) ? 'overlay-lane' : ''}`} style={{ backgroundSize: `${tickStep * scale}px 100%` }}>{row.items.filter(inView).map(item => {
    ...
    const selectedItem = selected?.id === item.id && (isOverlay ? selected.type === row.kind : selected.type === 'clip');
    return <button key={item.id} className={`timeline-clip ${row.kind} ${isOverlay ? 'overlay-clip' : ''} ${selectedItem ? 'selected' : ''} ${item.muted ? 'muted' : ''}`} style={{ left: item.start * scale, width: Math.max(item.duration * scale - 2, 5) }} data-action={`${isOverlay ? row.kind : 'timeline'}.selectAndDrag`} data-entity-id={item.id} aria-label={`${label}、${item.start}秒から${item.duration}秒`} title={`${label} · ${item.duration.toFixed(2)}s${row.kind === 'graphic' ? ` · ${graphicNames[item.type] || item.type}` : ''}`} onPointerDown={event => drag(event, item, row.kind)} onClick={() => selectItem(item, row.kind)}><div className="clip-name"><row.Icon size={10} /><span>{label}</span>{item.muted && <VolumeX size={10} />}</div>{!isOverlay && (row.kind === 'audio' ? <Waveform asset={asset} clip={item} /> : <div className="video-strip">{asset?.kind === 'image' && <img src={asset.url} loading="lazy" alt="" />}</div>)}{!isOverlay && <span className="clip-duration">{item.duration.toFixed(1)}s</span>}</button>;
