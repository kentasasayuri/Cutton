...
export default memo(function Timeline({ state, clock, selected, onSelect, onUpdate, onSplit, onDelete, onArrange, onClear, onOverlaySelect, onOverlayUpdate, onOverlayDelete, run, busy }) {
  ...
  const drag = (event, item, kind) => {
    ...
    const finish = current => {
      ...
      if(!current?.shiftKey){const points=[0,clock.time,...(state.markers||[]).map(m=>m.time),...[...state.clips,...state.captions,...state.graphics].filter(c=>c.id!==item.id).flatMap(c=>[c.start,c.start+c.duration])];const targets=points.flatMap(t=>[t,t-item.duration]).filter(t=>t>=0&&Math.abs(t-start)*scale<8).sort((a,b)=>Math.abs(a-start)-Math.abs(b-start));if(targets.length)start=targets[0];}
  ...
  return <section className="timeline-panel"><div className="timeline-toolbar"><div className="timeline-tools"><span className="sequence-label">タイムライン</span><span className="toolbar-divider" /><Button icon={Scissors} action="timeline.split" title="再生ヘッドで分割 (S)" aria-label="再生ヘッドで分割" disabled={!selectedClip || !!busy} onClick={() => onSplit(selectedClip)} /><Button icon={Trash2} action="timeline.removeSelected" title="選択レイヤーを削除" aria-label="選択レイヤーを削除" disabled={(!selectedClip && !overlaySelected) || !!busy} onClick={remove} /><span className="toolbar-divider" /><Button icon={Layers3} action="timeline.arrange.open" disabled={!state.assets.some(a => a.decision === 'accepted' && a.sceneId) || !!busy} onClick={onArrange}>採用素材を整列</Button></div><div className="timeline-tools"><Button icon={RefreshCw} action="waveform.retry" title="波形を再取得" aria-label="波形を再取得" disabled={!state.clips.some(clip => clip.track === 'audio')} onClick={retryWaveforms} /><span className="timeline-duration">{timecode(clock.duration, state.fps)}</span><label className="zoom-control"><span>−</span><input type="range" aria-label="タイムラインのズーム" data-action="timeline.zoom" min="12" max="90" step="2" value={scale} onChange={event => setScale(Number(event.target.value))} /><span>+</span></label><Button icon={Trash2} title="映像と音声の配置を空にする" aria-label="映像と音声の配置を空にする" action="timeline.clear.open" disabled={!state.clips.length || !!busy} onClick={onClear} /></div></div><TimingTools state={state} clock={clock} items={chosen} run={run} busy={busy} onClear={()=>setGroup([])} onSelectAll={()=>setGroup([...state.clips.map(i=>({id:i.id,type:'clip'})),...state.captions.map(i=>({id:i.id,type:'caption'})),...state.graphics.map(i=>({id:i.id,type:'graphic'}))])}/><div className="timeline-scroller" ref={scrollRef}><div className="timeline-content" style={{ minWidth: width + 96, minHeight: 28 + rows.reduce((sum, row) => sum + (['graphic', 'caption'].includes(row.kind) ? 44 : 74), 0) }}><div className="timeline-labels"><div className="ruler-label">秒</div>{rows.map(row => <div key={row.id} className={['graphic', 'caption'].includes(row.kind) ? 'overlay-label' : ''}><span className={`track-letter ${row.kind}-letter`}>{row.label}</span><span>{row.title}</span><row.Icon size={11} /></div>)}</div><div className="timeline-lanes" ref={laneRef} style={{ width }} onPointerDown={seek}><div className="timeline-ruler">{ticks.map(tick => <span key={tick} style={{ left: tick * scale }}>{timecode(tick).slice(3, 8)}</span>)}</div>{rows.map(row => <div key={row.id} className={`timeline-lane ${row.kind}-lane ${['graphic', 'caption'].includes(row.kind) ? 'overlay-lane' : ''}`} style={{ backgroundSize: `${tickStep * scale}px 100%` }}>{row.items.filter(inView).map(item => {
