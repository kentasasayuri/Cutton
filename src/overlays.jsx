...
function OverlayForm({ item, kind, state, clock, run, busy, onSelect, onNew }) {
  ...
  const save = async event => {
    ...
    if(caption)Object.assign(fields,Object.fromEntries(Object.keys(captionDefaults).map(key=>[key,draft[key]??captionDefaults[key]])));
  ...
  return <form className="overlay-form" onSubmit={save}>
    ...
    {caption&&<CaptionFields draft={draft} set={set} clock={clock}/>}
    {!caption&&['vector','null'].includes(draft.type)&&<VectorFields draft={draft} set={set} state={state}/>}
    {!caption&&<MotionFields draft={draft} set={set}/>}
...
export const OverlayPreview = memo(function OverlayPreview({ state, clock, selected, onSelect }) {
  ...
  const items = useMemo(() => [...(state.graphics || []).filter(item=>!['vector','null'].includes(item.type)).map(item => ({ ...item, kind: 'graphic' }))], [state.graphics]);
  useEffect(() => {
  ...
  })}<CaptionPreview state={state} clock={clock} onSelect={onSelect}/></div>;
});
