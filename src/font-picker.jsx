...
import {ARTICLE_FONTS,BUNDLED_FONTS,EXTRA_FONTS,FONT_ARTICLE} from '../server/font-catalog.mjs';
...
export default function FontPicker({value,onChange,text='伝えたい言葉を、読みやすく。'}){
 ...
 const available=[...['Meiryo','Yu Gothic','Yu Mincho','Arial'].map(family=>({family,label:family,group:'標準',ready:true})),...[...ARTICLE_FONTS,...EXTRA_FONTS].map(f=>({...f,ready:!!f.files.length||custom.some(c=>c.family===f.family)})),...custom.filter(f=>![...ARTICLE_FONTS,...EXTRA_FONTS].some(a=>a.family===f.family)).map(f=>({...f,label:f.family,group:'取り込み',ready:true}))];
