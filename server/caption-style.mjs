...
export const CAPTION_FONTS=['Meiryo','Yu Gothic','Yu Mincho','Arial'];
export const captionDefaults={fontFamily:'Meiryo',fontWeight:700,outline:2,outlineColor:'#101820',opacity:1,gradient:false,gradientColor:'#53e0ed',gradientAngle:90,tracking:0,lineHeight:1.3,shadow:1,background:false,backgroundColor:'#101820',backgroundOpacity:.65,padding:16,cornerRadius:8,textAnimation:'none',animationDuration:.6,stagger:.05,karaoke:'none',highlightColor:'#ffdb58',words:[]};
...
export function captionStyle(args,duration,text){
 ...
 if(!CAPTION_FONTS.includes(v.fontFamily))throw new Error('未対応のフォントです。');result.fontFamily=v.fontFamily;
 for(const [key,min,max] of [['fontWeight',100,900],['outline',0,30],['opacity',0,1],['gradientAngle',-360,360],['tracking',-10,40],['lineHeight',.8,3],['shadow',0,30],['backgroundOpacity',0,1],['padding',0,100],['cornerRadius',0,100],['animationDuration',.05,10],['stagger',0,1]])result[key]=finite(v[key],min,max,key);
 for(const key of ['outlineColor','gradientColor','backgroundColor','highlightColor'])result[key]=hex(v[key]);
 for(const key of ['gradient','background']){if(typeof v[key]!=='boolean')throw new Error(`${key} must be boolean`);result[key]=v[key];}
 for(const [key,choices] of [['textAnimation',['none','typewriter','fade-letters','rise-letters','pop-letters','wave']],['karaoke',['none','sweep','words']]]){if(!choices.includes(v[key]))throw new Error(`Invalid ${key}`);result[key]=v[key];}
...
export function captionsSvg(state,time){
 ...
 content+=`<defs><filter id="${id}s" x="-50%" y="-100%" width="200%" height="300%"><feDropShadow dx="0" dy="${v.shadow}" stdDeviation="${v.shadow/2}" flood-opacity=".65"/></filter></defs><g data-caption-id="${esc(v.id)}" transform="translate(${width*v.x/100} ${height*v.y/100})">`;
