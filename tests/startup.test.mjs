...
test('Sites links open the editor shell while cross-site project/media access stays closed',async()=>{
 ...
 try{
  for(const url of ['/?launch=projects','/index.html'])for(const origin of [undefined,SITE_ORIGIN,'https://external.example']){
  ...
  for(const url of ['/api/state','/api/health','/api/events','/api/download?path=state.json','/media/test','/thumbnail/test','/proxy/test']){
   ...
   assert.equal((await get(base,url,{...navigation,Origin:SITE_ORIGIN})).status,401,url);
  }
