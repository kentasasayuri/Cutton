...
test('one multipart font imports, serves identical bytes through Sites, and rejects malformed data',async()=>{
 ...
 const pairing=await(await fetch(base+'/api/pair',{method:'POST',headers:{Origin:SITE_ORIGIN}})).json();r=await fetch(base+font.url+'?access_token='+pairing.token,{headers:{Origin:SITE_ORIGIN}});assert.equal(r.status,200);assert.equal(r.headers.get('access-control-allow-origin'),SITE_ORIGIN);assert.deepEqual(Buffer.from(await r.arrayBuffer()),bytes);
